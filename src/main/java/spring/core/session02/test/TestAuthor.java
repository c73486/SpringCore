...
public class TestAuthor {
	...
	public static void main(String[] args) {
		...
		System.out.println(author3);//透過設定檔方法注入自動注入資料
		...
		Map<Character, Double> avgAgeBySex = authors.stream()
												 .collect(Collectors.groupingBy(
														 Author::getSex,
														 Collectors.averagingInt(Author::getAge)
														 ));
		System.out.println(avgAgeBySex);
		avgAgeBySex.forEach((sex,age) -> {
			System.out.printf("性別: %c, 平均年齡: %.1f\n",sex,age);
		});
